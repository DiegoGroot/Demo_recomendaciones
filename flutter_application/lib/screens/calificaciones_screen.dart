...
class _CalificacionesScreenState extends State<CalificacionesScreen> {
  ...
  List<Calificacion> get _filtradas {
    ...
    return _cals
        .where((c) =>
            _nombreEst(c.estudianteId).toLowerCase().contains(q) ||
            _nombreMat(c.materiaId).toLowerCase().contains(q))
        .toList();
  }
  ...
  void _abrirFormulario({Calificacion? cal}) {
    ...
    final semCtrl =
        TextEditingController(text: cal?.semestre?.toString() ?? '1');
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setS) => AlertDialog(
          ...
          title: Row(children: [
            Icon(cal == null ? Icons.add_circle : Icons.edit,
                color: Colors.indigo.shade700),
            const SizedBox(width: 8),
            Text(cal == null ? 'Nueva Calificación' : 'Editar Calificación',
              ...
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: const [
                    ...
                    DropdownMenuItem(
                        value: 'reprobado', child: Text('Reprobado')),
                  ],
            ...
            ElevatedButton.icon(
              ...
              onPressed: saving
                  ...
                  : () async {
                      ...
                      if (estId == null ||
                          matId == null ||
                          fnCtrl.text.trim().isEmpty) {
                        ScaffoldMessenger.of(context)
                            .showSnackBar(const SnackBar(
                          content: Text(
                              'Estudiante, materia y nota final son obligatorios'),
                          backgroundColor: Colors.red,
                        ));
                        return;
                      }
                      final notaFinal = double.tryParse(fnCtrl.text.trim());
                      if (notaFinal == null || notaFinal < 0 || notaFinal > 5) {
                        ScaffoldMessenger.of(context)
                            .showSnackBar(const SnackBar(
                          content:
                              Text('La nota final debe estar entre 0.0 y 5.0'),
                      ...
                      try {
                        final nueva = Calificacion(
                          estudianteId: estId!,
                          materiaId: matId!,
                          notaParcial1: double.tryParse(p1Ctrl.text.trim()),
                          notaParcial2: double.tryParse(p2Ctrl.text.trim()),
                          notaFinal: notaFinal,
                          semestre: int.tryParse(semCtrl.text.trim()) ?? 1,
                          estado: estado,
                        );
                        if (cal != null) {
                          await ApiService.updateCalificacion(
                              cal.calificacionId!, nueva);
                        } else {
                  ...
                  Container(
                    ...
                    child: Row(children: [
                      ...
                      Container(
                        ...
                        child: Text('${_filtradas.length} registros',
                            ...
                            child: SingleChildScrollView(
                              ...
                              child: DataTable(
                                ...
                                columns: const [
                                  ...
                                  DataColumn(
                                      label: Text('Parcial 1'), numeric: true),
                                  DataColumn(
                                      label: Text('Parcial 2'), numeric: true),
                                  DataColumn(
                                      label: Text('Nota Final'), numeric: true),
                                  DataColumn(
                                      label: Text('Semestre'), numeric: true),
                                ...
                                rows: _filtradas.asMap().entries.map((entry) {
                                  ...
                                  return DataRow(
                                    ...
                                    cells: [
                                      ...
                                      DataCell(SizedBox(
                                        width: 130,
                                        child: Text(_nombreMat(c.materiaId),
                                            overflow: TextOverflow.ellipsis),
                                      )),
                                      DataCell(Text(
                                          ...
                                          style: TextStyle(
                                              color:
                                                  _colorNota(c.notaParcial1)))),
                                      DataCell(Text(
                                          c.notaParcial2?.toStringAsFixed(1) ??
                                              '—',
                                          style: TextStyle(
                                              color:
                                                  _colorNota(c.notaParcial2)))),
                                      DataCell(Container(
                                        padding: const EdgeInsets.symmetric(
                                            horizontal: 10, vertical: 4),
                                        decoration: BoxDecoration(
                                          color:
                                              colorNota.withValues(alpha: 0.12),
                                          borderRadius:
